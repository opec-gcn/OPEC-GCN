...
def eval_map(alpha_pose_generator,model_pose,test_dataloader,pred_json,best_json,target_json):
    ...
    with open(pred_json) as reader:
        ...
        json_file_0_1 = copy.deepcopy(json_file_ori)
        json_file_0_05 = copy.deepcopy(json_file_ori)
        json_file_0_03 = copy.deepcopy(json_file_ori)
        json_file_0_04 = copy.deepcopy(json_file_ori)
        json_file_0_02 = copy.deepcopy(json_file_ori)
        inter_gral_result = copy.deepcopy(json_file_ori)
    ...
    for batches in tqdm(test_dataloader):
        ...
        with torch.no_grad():
            ...
            dts_01[labels<0.1] = adj_joints[labels<0.1]
            dts_005[labels<0.3] = adj_joints[labels<0.3]
            dts_004[labels<0.25] = adj_joints[labels<0.25]
            dts_003[labels<0.2] = adj_joints[labels<0.2]
            dts_002[labels < 0.15] = adj_joints[labels < 0.15]
            for bz in range(dts_002.shape[0]):
                index = item[bz]
                name = img_name[bz]
                ...
                json_file_0_1[index]['keypoints'] = dts_01[bz,...].reshape(-1).tolist()
                json_file_0_05[index]['keypoints'] = dts_005[bz,...].reshape(-1).tolist()
                json_file_0_04[index]['keypoints'] = dts_004[bz,...].reshape(-1).tolist()
                json_file_0_03[index]['keypoints'] = dts_003[bz,...].reshape(-1).tolist()
                json_file_0_02[index]['keypoints'] = dts_002[bz,...].reshape(-1).tolist()
                inter_gral_result[index]['keypoints'] = inter_gral_x[bz,...].reshape(-1).tolist()
    new_best_match =[]
    ...
    # __ap,_,_ = eval_results(new_best_match,target_json)
    # ap_intergral,_,_ = eval_results(inter_gral_result,target_json)

    ap,_,_ = eval_results(json_file_ori,target_json)
    ap01,_,_ = eval_results(json_file_0_1,target_json)
    ap005,_,_ = eval_results(json_file_0_05,target_json)
    ap004,_,_ = eval_results(json_file_0_04,target_json)
    ap003,_,_ = eval_results(json_file_0_03,target_json)
    ap002,_,_ = eval_results(json_file_0_02,target_json)
    return max(ap,ap01,ap005,ap003,ap004,ap002),ap